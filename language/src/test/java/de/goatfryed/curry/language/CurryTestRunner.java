...
public class CurryTestRunner extends ParentRunner<CurryTestRunner.TestCase> {

    private static final String SOURCE_SUFFIX = CurryLanguage.DEFAULT_EXTENSION;
    ...
    public CurryTestRunner(Class<?> runningClass) throws InitializationError {
        super(runningClass);
        try {
    ...
    protected static List<TestCase> createTests(final Class<?> c) throws IOException, InitializationError {
        CurryTestSuit suite = c.getAnnotation(CurryTestSuit.class);
        if (suite == null) {
            throw new InitializationError(String.format("@%s annotation required on class '%s' to run with '%s'.", CurryTestSuit.class.getSimpleName(), c.getName(), CurryTestSuit.class.getSimpleName()));
        }
        ...
        if (suite.testCaseDirectory() != CurryTestSuit.class) {
    ...
    protected void runChild(TestCase testCase, RunNotifier notifier) {
        var prevLoader = Thread.currentThread().getContextClassLoader();
        Thread.currentThread().setContextClassLoader(getClass().getClassLoader());
        runChildSafe(testCase, notifier);
        Thread.currentThread().setContextClassLoader(prevLoader);
    }

    private void runChildSafe(TestCase testCase, RunNotifier notifier) {
    ...
    private static void run(Context context, Path path, PrintWriter out) throws IOException {
        try {
            ...
            Source source = Source.newBuilder(CurryLanguage.ID, path.toFile())
                .interactive(true)
                .build();
    ...
    public static void runInMain(Class<?> testClass, String[] args) throws InitializationError, NoTestsRemainException {
        ...
        CurryTestRunner suite = new CurryTestRunner(testClass);
        if (args.length > 0) {
